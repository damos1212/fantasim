...
export class Simulation {
  ...
  getInitialMessage() {
    return {
      ...
      world: {
        ...
        road: this.world.road,
        owner: this.world.owner,
        resourceType: this.world.resourceType,
  ...
  private seedTribes(): void {
    ...
    for (let tribeId = 0; tribeId < selectedRaces.length; tribeId += 1) {
      ...
      tribe.resources[ResourceType.Stone] = 6;
      tribe.resources[ResourceType.Grain] = 2;
      tribe.resources[ResourceType.Clay] = 2;
      tribe.resources[ResourceType.Rations] = 6;
      tribe.resources[ResourceType.Berries] = 2;
      tribe.resources[ResourceType.Meat] = 1;
  ...
  private seedStarterStockyard(tribe: TribeState, stockpile: BuildingState): void {
    ...
    for (let row = 0; row < layout.length; row += 1) {
      for (let col = 0; col < layout[row]!.length; col += 1) {
        ...
        this.world.resourceType[index] = resourceType;
        this.world.resourceAmount[index] = resourceType === ResourceType.Wood ? 8
          : resourceType === ResourceType.Stone ? 6
          : resourceType === ResourceType.Clay ? 4
          : resourceType === ResourceType.Grain ? 6
          : 5;
      }
    }
  }
    ...
    const stockTargets: Array<{ building: BuildingState; resources: Array<[ResourceType, number]> }> = [
      ...
      },
      {
        ...
        resources: [
          ...
          [ResourceType.Rations, 4],
          [ResourceType.Meat, 1],
          [ResourceType.StoneTools, 2],
          [ResourceType.BasicWeapons, 1],
  ...
  private farmHarvestTargetForSite(site: BuildingState): number {
    return site.type === BuildingType.Orchard ? 18 : 24;
  }

  private countHarvestNodesNearSite(site: BuildingState, resourceType: ResourceType): number {
    const center = buildingCenter(site);
    let total = 0;
    for (let dy = -4; dy <= 4; dy += 1) {
      for (let dx = -4; dx <= 4; dx += 1) {
        ...
        if (!inBounds(x, y, this.world.width, this.world.height)) continue;
        const index = indexOf(x, y, this.world.width);
        if (this.world.resourceType[index] === resourceType && this.world.resourceAmount[index] > 0) {
          total += this.world.resourceAmount[index]!;
        }
      }
    }
    return total;
  ...
  private spawnFarmHarvestNodes(tribe: TribeState, weather: WeatherKind): void {
    const floodedFields = this.floodedFarmCountForTribe(tribe.id);
    const waterSupport = this.surfaceWaterFarmSupportForTribe(tribe.id) + this.cisternBonusForTribe(tribe.id);
    for (const site of this.buildingsForTribe(tribe.id)) {
      if (site.type !== BuildingType.Farm && site.type !== BuildingType.Orchard) continue;
      const resourceType = site.type === BuildingType.Orchard ? ResourceType.Berries : ResourceType.Grain;
      const current = this.countHarvestNodesNearSite(site, resourceType);
      const target = this.farmHarvestTargetForSite(site);
      if (current >= target) continue;

      const weatherFactor =
        weather === WeatherKind.Heatwave ? 0.55
        : weather === WeatherKind.Blizzard ? 0.7
        : weather === WeatherKind.Rain ? 1.15
        : weather === WeatherKind.Storm ? 0.9
        : 1;
      const supportBonus = Math.min(8, Math.floor(waterSupport * 0.35));
      const floodedPenalty = Math.min(8, floodedFields * 2);
      const spawnAmount = Math.max(
        0,
        Math.floor((((site.type === BuildingType.Orchard ? 7 : 9) + supportBonus - floodedPenalty) * weatherFactor) * tribe.race.foodBias),
      );
      ...
      for (let dy = -4; dy <= 4; dy += 1) {
        for (let dx = -4; dx <= 4; dx += 1) {
          ...
          if (this.world.owner[index] !== tribe.id) continue;
          const terrain = this.world.terrain[index] as TerrainType;
          if (terrain === TerrainType.WaterDeep || terrain === TerrainType.WaterShallow || terrain === TerrainType.Lava || terrain === TerrainType.Mountain) continue;
          if (this.world.resourceAmount[index] > 0 && this.world.resourceType[index] !== resourceType) continue;
          candidates.push(index);
        }
      }
      if (candidates.length === 0) continue;

      let remaining = Math.min(spawnAmount, target - current);
      while (remaining > 0 && candidates.length > 0) {
        const pick = randInt(this.random, 0, candidates.length - 1);
        const index = candidates.splice(pick, 1)[0]!;
        const chunk = Math.min(remaining, randInt(this.random, 3, 7));
        this.world.resourceType[index] = resourceType;
        this.world.resourceAmount[index] = clamp(this.world.resourceAmount[index]! + chunk, 0, 48);
        this.world.feature[index] = site.type === BuildingType.Orchard ? FeatureType.BerryPatch : FeatureType.None;
        this.markDirty(index);
        remaining -= chunk;
      }
    }
  ...
  private consumeAndGrow(): void {
    for (const tribe of this.tribes) {
      ...
      if (this.tickCount % (SIM_TICKS_PER_SECOND * 6) === 0) {
        ...
        this.spawnFarmHarvestNodes(tribe, weather);
  ...
  private runTribeStrategy(): void {
    ...
    for (const tribe of this.tribes) {
      ...
      if (!bootstrap) {
        ...
        this.generateFrontierExpansionPlans(tribe);
  ...
  private generateEconomyJobs(tribe: TribeState, bootstrap = false): void {
    ...
    if (farms.length > 0) {
      ...
      for (const farm of farms) {
        ...
        const center = buildingCenter(farm);
        activeFarmJobs += this.enqueueResourceJobsAround(tribe, center.x, center.y, [farmResource], "farm", 7, Math.max(1, desiredFarmJobs - activeFarmJobs));
      }
    }
  ...
  private generateDistrictPlans(tribe: TribeState): void {
    ...
    const remoteCandidates = this.buildingsForTribe(tribe.id)
      ...
      .filter((entry) => entry.storageDistance >= 8 || entry.top.amount >= 16)
      .sort((a, b) =>
    ...
    for (const target of targets) {
      ...
      const productiveRemote = target.top.amount >= 10 || target.storageDistance >= 8;
      ...
      const branchPull = target.top.amount * 0.35 + target.storageDistance * 0.9 + target.roadScore * 3 + supportCount * 1.2;
      ...
      if (
        ...
        population >= (specialization === "mining" || specialization === "harbor" ? 18 : 20) &&
        (target.storageDistance >= (specialization === "mining" ? 12 : 14) || target.top.amount >= (specialization === "agriculture" ? 28 : 34) || branchPull >= 24) &&
      ...
      if (
        productiveRemote &&
        (
          (population >= 12 && target.storageDistance >= 7 && (target.top.amount >= 8 || target.roadScore >= 2 || supportCount >= 2))
          || (population >= 18 && (target.storageDistance >= 9 || target.top.amount >= 16 || branchPull >= 22))
        ) &&
        !nearbyHall &&
        !nearbyPlannedHall &&
      ...
      if (
        tribe.age >= AgeType.Stone &&
        population >= 20 &&
        (specialization === "industry" || specialization === "mining" || target.top.amount >= 28) &&
      ...
      if (
        ...
        supportCount >= 3 &&
        nearbyWarehouses === 0 &&
        (target.storageDistance >= 9 || target.top.amount >= 20 || specialization === "mining" || specialization === "industry" || branchPull >= 22) &&
      ...
      if (
        ...
        supportCount >= 2 &&
        nearbyWorkshops === 0 &&
        (specialization === "mining" || specialization === "industry" || target.top.amount >= 18 || branchPull >= 20) &&
        !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Workshop, target.center.x, target.center.y, 8)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Workshop, 6, target.center.x, target.center.y, 8);
      }
    ...
    const hallTarget = targets.find((target) => {
      ...
      return !nearbyHall && target.storageDistance >= 6 && (target.top.amount >= 8 || target.roadScore >= 2 || tribeBuildingTotal >= 9);
    });
    if (
      hallTarget &&
      population >= 16 &&
      tribeBuildingTotal >= 8 &&
      this.buildingCount(tribe.id, BuildingType.CapitalHall) < this.maxBuildingCountForTribe(tribe, BuildingType.CapitalHall) &&
    ...
    if (
      ...
      const fallbackBranchTarget = this.buildingsForTribe(tribe.id)
        .filter((building) =>
          building.type === BuildingType.Farm
          || building.type === BuildingType.Orchard
          || building.type === BuildingType.LumberCamp
        ...
        .map((building) => {
          const center = buildingCenter(building);
          const top = this.topStoredResource(building);
          const hallDistance = this.capitalHallsForTribe(tribe.id)
            .reduce((min, hall) => Math.min(min, manhattan(center.x, center.y, buildingCenter(hall).x, buildingCenter(hall).y)), Number.POSITIVE_INFINITY);
          return {
            building,
            center,
            top,
            hallDistance,
            score:
              Math.max(0, top.amount - 8) * 1.2
              + Math.max(0, this.nearestStorageDistance(tribe.id, center.x, center.y) - 7) * 1.6
              + Math.max(0, manhattan(center.x, center.y, tribe.capitalX, tribe.capitalY) - 13) * 0.8
              + this.nearbyRoadScore(center.x, center.y, 2) * 3
              + Math.max(0, hallDistance - 11) * 2.1,
          };
        })
        .filter((entry) =>
          entry.hallDistance >= 12
          && manhattan(entry.center.x, entry.center.y, tribe.capitalX, tribe.capitalY) >= 12
          && (entry.top.amount >= 6 || this.nearestStorageDistance(tribe.id, entry.center.x, entry.center.y) >= 6),
        )
        .sort((a, b) => b.score - a.score)[0];
      if (
        fallbackBranchTarget &&
        !this.hasNearbyPlannedBuild(tribe.id, BuildingType.CapitalHall, fallbackBranchTarget.center.x, fallbackBranchTarget.center.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.CapitalHall, 12, fallbackBranchTarget.center.x, fallbackBranchTarget.center.y, 10);
      }
    }
  ...
  private generateFrontierExpansionPlans(tribe: TribeState): void {
    const population = this.populationOf(tribe.id);
    if (population < 18) {
      return;
    }
    ...
    for (const hall of this.capitalHallsForTribe(tribe.id)) {
      const center = buildingCenter(hall);
      const localWood = this.nearbyResourceAmount(center.x, center.y, 12, (resourceType, feature) =>
        resourceType === ResourceType.Wood || feature === FeatureType.Trees,
      );
      const localStone = this.nearbyResourceAmount(center.x, center.y, 12, (resourceType, feature) =>
        resourceType === ResourceType.Stone || resourceType === ResourceType.Clay || feature === FeatureType.StoneOutcrop || feature === FeatureType.ClayDeposit,
      );
      const localOre = this.nearbyResourceAmount(center.x, center.y, 16, (resourceType, feature) =>
        resourceType === ResourceType.Ore || feature === FeatureType.OreVein,
      );
      const localFoodNodes = this.nearbyResourceAmount(center.x, center.y, 10, (resourceType, feature) =>
        resourceType === ResourceType.Grain
        || resourceType === ResourceType.Berries
        || resourceType === ResourceType.Fish
        || feature === FeatureType.BerryPatch
        || feature === FeatureType.FishShoal,
      );
      ...
      const localStoneStock =
        this.hallStoredAmount(tribe.id, hall, ResourceType.Stone)
        + this.hallStoredAmount(tribe.id, hall, ResourceType.Clay);
      const remoteRadius = population >= 42 ? 28 : population >= 28 ? 24 : 20;

      if (
        (localFoodNodes < 18 || localFoodStock < Math.max(18, population * 1.6)) &&
        this.nearbyBuildingCount(tribe.id, BuildingType.Farm, center.x, center.y, 12) < 2 &&
        !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Farm, center.x, center.y, remoteRadius)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Farm, 8, center.x, center.y, remoteRadius);
        if (tribe.race.personality.ecology > 0.6 && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Orchard, center.x, center.y, remoteRadius)) {
          this.tryPlanBuildingAround(tribe, BuildingType.Orchard, 6, center.x, center.y, remoteRadius);
        }
      }

      if (
        (localWood < 18 || localWoodStock < 18) &&
        this.nearbyBuildingCount(tribe.id, BuildingType.LumberCamp, center.x, center.y, 12) < 2 &&
        !this.hasNearbyPlannedBuild(tribe.id, BuildingType.LumberCamp, center.x, center.y, remoteRadius)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.LumberCamp, 8, center.x, center.y, remoteRadius);
      }

      if (
        (localStone < 16 || localStoneStock < 14) &&
        this.nearbyBuildingCount(tribe.id, BuildingType.Quarry, center.x, center.y, 12) < 2 &&
        !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Quarry, center.x, center.y, remoteRadius)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Quarry, 7, center.x, center.y, remoteRadius);
      }

      if (
        tribe.age >= AgeType.Stone &&
        (localOre < 14 || tribe.resources[ResourceType.Ore] < 16) &&
        this.nearbyBuildingCount(tribe.id, BuildingType.Mine, center.x, center.y, 14) === 0 &&
        !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Mine, center.x, center.y, remoteRadius)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Mine, 7, center.x, center.y, remoteRadius);
      }

      if (
        (localFoodNodes < 20 || localWood < 20 || localStone < 18) &&
        this.nearbyBuildingCount(tribe.id, BuildingType.Stockpile, center.x, center.y, 10) === 0 &&
        !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Stockpile, center.x, center.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Stockpile, 7, center.x, center.y, 10);
      }
    }
  }
  ...
  private generateBranchHubPlans(tribe: TribeState): void {
    ...
    for (const hall of halls) {
      ...
      const nearbyOrchards = this.nearbyBuildingCount(tribe.id, BuildingType.Orchard, center.x, center.y, 10);
      ...
      const nearbyMines = this.nearbyBuildingCount(tribe.id, BuildingType.Mine, center.x, center.y, 10)
        + this.nearbyBuildingCount(tribe.id, BuildingType.DeepMine, center.x, center.y, 12);
      ...
      const localFoodNodes = this.nearbyResourceAmount(center.x, center.y, 11, (resourceType, feature) =>
        resourceType === ResourceType.Grain
        || resourceType === ResourceType.Berries
        || resourceType === ResourceType.Fish
        || feature === FeatureType.BerryPatch
      ...
      const localWoodNodes = this.nearbyResourceAmount(center.x, center.y, 12, (resourceType, feature) =>
        resourceType === ResourceType.Wood || feature === FeatureType.Trees,
      );
      const localStoneNodes = this.nearbyResourceAmount(center.x, center.y, 12, (resourceType, feature) =>
        resourceType === ResourceType.Stone
        || resourceType === ResourceType.Clay
        || feature === FeatureType.StoneOutcrop
        || feature === FeatureType.ClayDeposit,
      );
      const localOreNodes = this.nearbyResourceAmount(center.x, center.y, 14, (resourceType, feature) =>
        resourceType === ResourceType.Ore || feature === FeatureType.OreVein,
      );
      const maturity = this.branchMaturityStage(tribe.id, hall);
      const strained =
      ...
      const shortFood = localFoodStock < foodTarget * 0.7 || localFoodNodes < Math.max(14, maturity >= 2 ? 18 : 16);
      const shortWood = localWoodStock < woodTarget * 0.72 || localWoodNodes < Math.max(16, maturity >= 2 ? 18 : 16);
      const shortStone = localStoneStock < stoneTarget * 0.72 || localStoneNodes < Math.max(12, maturity >= 2 ? 14 : 12);
      const shortOre = tribe.age >= AgeType.Stone
        && (
          this.hallStoredAmount(tribe.id, hall, ResourceType.Ore) < this.hallLocalResourceTarget(tribe.id, hall, ResourceType.Ore) * 0.55
          || localOreNodes < Math.max(8, maturity >= 2 ? 10 : 8)
        );
      const branchPressure =
        this.hallLogisticsDemandScore(tribe.id, hall, ResourceType.Rations)
        + this.hallLogisticsDemandScore(tribe.id, hall, ResourceType.Wood)
        + this.hallLogisticsDemandScore(tribe.id, hall, ResourceType.Stone);
      ...
      if (
        (maturity >= 2 || (maturity >= 1 && branchPressure >= 2.2) || strained) &&
        tribe.age >= AgeType.Stone
        && nearbyWarehouses === 0
        && population >= 16
        && nearbyProductiveSite
        && (top.amount >= 12 || branchPressure >= 2.4)
      ...
      if (
        (maturity >= 2 || branchPressure >= 2.6) &&
        tribe.age >= AgeType.Stone
        && nearbyWorkshops === 0
        && nearbyProductiveSite
        && (top.amount >= 14 || branchPressure >= 2.8)
        && !(
          specialization === "mining"
          && tribe.age >= AgeType.Iron
          && (race === RaceType.Dwarves || race === RaceType.Darkfolk || race === RaceType.Goblins)
      ...
      if (
        shortFood
        && nearbyFarms + nearbyOrchards < (maturity >= 2 ? 3 : 2)
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Farm, center.x, center.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Farm, strained ? 8 : 6, center.x, center.y, 10);
      }
      if (
        shortFood
        && maturity >= 2
        && (race === RaceType.Elves || race === RaceType.Halflings || tribe.race.personality.ecology > 0.58)
        && nearbyOrchards === 0
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Orchard, center.x, center.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Orchard, 5, center.x, center.y, 10);
      }
      ...
      if (
        shortWood
        && nearbyLumber < (maturity >= 2 ? 2 : 1)
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.LumberCamp, center.x, center.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.LumberCamp, strained ? 8 : 6, center.x, center.y, 10);
      }
      ...
      if (
        ...
        && tribe.age >= AgeType.Stone
        && shortStone
        && nearbyQuarries < (maturity >= 2 ? 2 : 1)
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Quarry, center.x, center.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Quarry, strained ? 7 : 5, center.x, center.y, 10);
      }
      if (
        tribe.age >= AgeType.Stone
        && shortOre
        && (specialization === "mining" || maturity >= 2 || localOreNodes >= 10)
        && nearbyMines === 0
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Mine, center.x, center.y, 12)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Mine, strained ? 7 : 5, center.x, center.y, 12);
      }
    ...
    for (const plan of plannedBranchHalls) {
      ...
      if (this.nearbyBuildingCount(tribe.id, BuildingType.Farm, plan.x, plan.y, 10) === 0
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Farm, plan.x, plan.y, 10)) {
        this.tryPlanBuildingAround(tribe, BuildingType.Farm, 6, plan.x, plan.y, 10);
      }
      if (this.nearbyBuildingCount(tribe.id, BuildingType.LumberCamp, plan.x, plan.y, 10) === 0
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.LumberCamp, plan.x, plan.y, 10)) {
        this.tryPlanBuildingAround(tribe, BuildingType.LumberCamp, 5, plan.x, plan.y, 10);
      }
      if (
        tribe.age >= AgeType.Stone
        && this.nearbyBuildingCount(tribe.id, BuildingType.Quarry, plan.x, plan.y, 10) === 0
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Quarry, plan.x, plan.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Quarry, 5, plan.x, plan.y, 10);
      }
      if (
        tribe.age >= AgeType.Stone
        && population >= 18
      ...
      if (
        tribe.age >= AgeType.Stone
        && population >= 18
        && this.nearbyBuildingCount(tribe.id, BuildingType.Workshop, plan.x, plan.y, 10) === 0
        && !this.hasNearbyPlannedBuild(tribe.id, BuildingType.Workshop, plan.x, plan.y, 10)
      ) {
        this.tryPlanBuildingAround(tribe, BuildingType.Workshop, 6, plan.x, plan.y, 9);
      }
    }
  }
  ...
  private nearbyResourceAmount(
    x: number,
    y: number,
    radius: number,
    predicate: (resourceType: ResourceType, feature: FeatureType) => boolean,
  ): number {
    let total = 0;
    for (let dy = -radius; dy <= radius; dy += 1) {
      for (let dx = -radius; dx <= radius; dx += 1) {
        ...
        const resourceType = this.world.resourceType[index] as ResourceType;
        const feature = this.world.feature[index] as FeatureType;
        if (!predicate(resourceType, feature)) continue;
        total += this.world.resourceAmount[index] ?? 0;
      }
    }
    return total;
  }
    ...
    for (let dy = 0; dy < building.height; dy += 1) {
      for (let dx = 0; dx < building.width; dx += 1) {
        ...
        if (type === BuildingType.Farm || type === BuildingType.Orchard) {
          ...
          this.world.feature[index] = FeatureType.None;
          this.world.resourceType[index] = ResourceType.None;
          this.world.resourceAmount[index] = 0;
        }
        this.world.owner[index] = tribeId;
        this.markDirty(index);
  ...
  private connectBuildingRoads(building: BuildingState): void {
    ...
    const entry = this.roadConnectionPoint(building, anchor.x, anchor.y);
    this.layRoad(anchor.x, anchor.y, entry.x, entry.y, building.tribeId);
    if (building.type === BuildingType.FishingHut || building.type === BuildingType.Dock) {
      const capitalEntry = this.roadConnectionPoint(building, tribe.capitalX, tribe.capitalY);
      this.layRoad(tribe.capitalX, tribe.capitalY, capitalEntry.x, capitalEntry.y, building.tribeId);
    }
  }

  private roadConnectionPoint(building: BuildingState, towardX: number, towardY: number): { x: number; y: number } {
    const center = buildingCenter(building);
    const west = { x: building.x - 1, y: center.y };
    const east = { x: building.x + building.width, y: center.y };
    const north = { x: center.x, y: building.y - 1 };
    const south = { x: center.x, y: building.y + building.height };
    const candidates = [west, east, north, south]
      .filter((point) => inBounds(point.x, point.y, this.world.width, this.world.height));
    candidates.sort((a, b) => manhattan(a.x, a.y, towardX, towardY) - manhattan(b.x, b.y, towardX, towardY));
