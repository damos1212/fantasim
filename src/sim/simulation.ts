...
type AgentTask =
  | {
      kind: "gather" | "hunt" | "fish" | "mine" | "quarry" | "cut_tree" | "tame_horse" | "tame_livestock" | "replant_tree" | "dungeon" | "delve";
      targetX: number;
      targetY: number;
...
type JobState = {
  ...
  payload?: BuildPayload | CraftPayload | AttackPayload | EarthworkPayload | HaulPayload;
};

const RESOURCE_SLOTS = 32;
const TECH_THRESHOLDS = [0, 90, 260, 560, 960, 1520, 2360, 3400];
...
export class Simulation {
  ...
  private seedAnimals(): void {
    ...
    for (let i = 0; i < INITIAL_ANIMAL_HERDS; i += 1) {
  ...
  private assignRolesForTribe(tribe: TribeState): void {
    ...
    const desiredWoodcutters = clamp(Math.floor(tribeAgents.length * 0.14), 2, 12);
    const desiredMiners = tribe.age >= AgeType.Bronze ? clamp(Math.floor(tribeAgents.length * 0.1 * tribe.race.buildBias + this.buildingCount(tribe.id, BuildingType.DeepMine) * 0.8), 2, 12) : 0;
    ...
    const desiredCrafters = tribe.age >= AgeType.Bronze ? clamp(Math.floor(tribeAgents.length * (this.hasBuilt(tribe.id, BuildingType.Armory) ? 0.1 : 0.08)), 1, 10) : 0;
    const desiredScholars = tribe.age >= AgeType.Bronze && (this.hasBuilt(tribe.id, BuildingType.Workshop) || this.hasBuilt(tribe.id, BuildingType.Castle) || this.hasBuilt(tribe.id, BuildingType.MageTower) || this.hasBuilt(tribe.id, BuildingType.ArcaneSanctum) || this.hasBuilt(tribe.id, BuildingType.School))
      ...
      : 0;
    const desiredBuilders = clamp(Math.floor(tribeAgents.length * 0.12 + tribe.race.buildBias * 2), 2, 10);
    const desiredHaulers = clamp(Math.floor(tribeAgents.length * (this.hasBuilt(tribe.id, BuildingType.Warehouse) ? 0.11 : 0.08) + this.jobs.filter((job) => job.tribeId === tribe.id && (job.kind === "build" || job.kind === "haul")).length * 0.05), 1, 10);
  ...
  private updateCaravans(): void {
    for (let index = this.caravans.length - 1; index >= 0; index -= 1) {
      ...
      if (caravan.task === CaravanTaskType.ToPartner) {
        ...
        const returnCargoType = this.chooseTradeCargo(partner);
        ...
        caravan.path = findPath(this.world, caravan.x, caravan.y, caravan.targetX, caravan.targetY);
        caravan.pathIndex = 0;
        const outboundCargoType = this.chooseTradeCargo(tribe);
  ...
  private updateLegendaryCreatures(): void {
    for (const creature of this.creatures) {
      ...
      if (nearestTribe && manhattan(nearestTribe.capitalX, nearestTribe.capitalY, creature.lairX, creature.lairY) < 140 && this.random() > 0.992) {
  ...
  private claimTask(agent: AgentState, tribe: TribeState): AgentTask | null {
    ...
    for (const job of jobs) {
      ...
      agent.path = findPath(this.world, agent.x, agent.y, job.x, job.y);
      agent.pathIndex = 0;
      switch (job.kind) {
        case "haul":
          return {
        ...
        default:
          return {
            ...
            targetY: job.y,
            stage: "toTarget",
            resourceType: this.resourceForJob(job.kind),
            amount: 0,
          };
      }
    }
  ...
  private processTask(agent: AgentState, tribe: TribeState): void {
    ...
    if ((task.kind === "gather" || task.kind === "cut_tree" || task.kind === "quarry" || task.kind === "mine" || task.kind === "fish" || task.kind === "hunt" || task.kind === "tame_horse" || task.kind === "tame_livestock" || task.kind === "replant_tree" || task.kind === "dungeon" || task.kind === "delve")) {
      ...
      if (agent.carrying === ResourceType.Fish || agent.carrying === ResourceType.Berries || agent.carrying === ResourceType.Meat || agent.carrying === ResourceType.Grain) {
        tribe.resources[ResourceType.Rations] += Math.ceil(agent.carryingAmount * 0.8);
      }
  ...
  private performResourceTask(
    tribe: TribeState,
    agent: AgentState,
    task: Extract<AgentTask, { kind: "gather" | "cut_tree" | "quarry" | "mine" | "fish" | "hunt" | "tame_horse" | "tame_livestock" | "replant_tree" | "dungeon" | "delve" }>,
  ): boolean {
  ...
  private consumeAndGrow(): void {
    for (const tribe of this.tribes) {
      ...
      if (this.tickCount % (SIM_TICKS_PER_SECOND * 6) === 0) {
        ...
        const deepMineYield = this.buildingCount(tribe.id, BuildingType.DeepMine);
        const delveYield = this.buildingCount(tribe.id, BuildingType.TunnelEntrance);
        ...
        if (deepMineYield > 0) {
          tribe.resources[ResourceType.Ore] += deepMineYield * (2 + powerPlants);
          tribe.resources[ResourceType.Stone] += deepMineYield;
        }
        if (delveYield > 0) {
          tribe.resources[ResourceType.Stone] += delveYield;
          if (this.tickCount % (SIM_TICKS_PER_SECOND * 18) === 0) {
            tribe.resources[ResourceType.Ore] += Math.max(1, Math.floor(delveYield / 2));
          }
        }
  ...
  private generateBuildingPlans(tribe: TribeState): void {
    ...
    if (tribe.age >= AgeType.Stone && population > 18 && this.buildingCount(tribe.id, BuildingType.Stockpile) < 2) {
    ...
    if (tribe.age >= AgeType.Stone && this.buildingCount(tribe.id, BuildingType.Cistern) < 1) {
      this.tryPlanBuilding(tribe, BuildingType.Cistern, 5);
    }
    if (tribe.age >= AgeType.Stone && population > 24 && tribe.water < Math.max(16, population * 0.55) && this.buildingCount(tribe.id, BuildingType.Cistern) < 3) {
      this.tryPlanBuilding(tribe, BuildingType.Cistern, 7);
    }

    if (tribe.age >= AgeType.Stone && !this.hasBuilt(tribe.id, BuildingType.Farm)) {
      this.tryPlanBuilding(tribe, BuildingType.Farm, 7);
    }
    if (tribe.age >= AgeType.Stone && !this.hasBuilt(tribe.id, BuildingType.LumberCamp)) {
      this.tryPlanBuilding(tribe, BuildingType.LumberCamp, 7);
    }
    if (tribe.age >= AgeType.Stone && !this.hasBuilt(tribe.id, BuildingType.Quarry)) {
    ...
    if (tribe.race.personality.ecology > 0.65 && tribe.age >= AgeType.Stone && this.buildingCount(tribe.id, BuildingType.Orchard) < 2) {
      this.tryPlanBuilding(tribe, BuildingType.Orchard, 5);
    }
    if (tribe.age >= AgeType.Stone && population > 20 && !this.hasBuilt(tribe.id, BuildingType.Shrine)) {
      this.tryPlanBuilding(tribe, BuildingType.Shrine, 5);
    }
    if (tribe.age >= AgeType.Bronze && !this.hasBuilt(tribe.id, BuildingType.Workshop)) {
      this.tryPlanBuilding(tribe, BuildingType.Workshop, 7);
    }
    if (tribe.age >= AgeType.Bronze && !this.hasBuilt(tribe.id, BuildingType.School)) {
      this.tryPlanBuilding(tribe, BuildingType.School, 6);
    }
    if (tribe.age >= AgeType.Bronze && population > 26 && !this.hasBuilt(tribe.id, BuildingType.Tavern)) {
    ...
    if (tribe.age >= AgeType.Bronze && this.meanHostility(tribe) > 20 && !this.hasBuilt(tribe.id, BuildingType.Barracks)) {
    ...
    if (tribe.age >= AgeType.Iron && this.meanHostility(tribe) > 12 && !this.hasBuilt(tribe.id, BuildingType.Armory)) {
      this.tryPlanBuilding(tribe, BuildingType.Armory, 7);
    }
    if (tribe.age >= AgeType.Iron && this.buildingCount(tribe.id, BuildingType.Watchtower) < 2) {
    ...
    if (tribe.age >= AgeType.Medieval && population > 36 && !this.hasBuilt(tribe.id, BuildingType.Castle)) {
  ...
  private generateEarthworkPlans(tribe: TribeState): void {
    ...
    if (tribe.age >= AgeType.Stone) {
      this.planFarmCanals(tribe, tribe.age >= AgeType.Iron ? 4 : 2);
    }

    const hostility = this.meanHostility(tribe);
    if (hostility > 10) {
      this.planDefensiveRing(tribe, Math.min(12, 6 + Math.floor(hostility * 0.05)));
    }
    if (hostility > 22 && tribe.age >= AgeType.Bronze) {
      this.planOuterTrenches(tribe, 6);
    }
  }
  ...
  private planDefensiveRing(tribe: TribeState, limit: number): void {
    ...
    for (let dx = -radius; dx <= radius && planned < limit; dx += 2) {
      for (const dy of [-radius, radius]) {
        const kind = Math.abs(dx) <= 2 ? "gate" : tribe.age >= AgeType.Iron ? "stone_wall" : "palisade";
    ...
    for (let dy = -radius + 2; dy <= radius - 2 && planned < limit; dy += 2) {
      for (const dx of [-radius, radius]) {
        const kind = Math.abs(dy) <= 2 ? "gate" : tribe.age >= AgeType.Iron ? "stone_wall" : "palisade";
        if (this.canPlaceEarthwork(tribe.capitalX + dx, tribe.capitalY + dy, kind)) {
          this.enqueueEarthworkJob(tribe, tribe.capitalX + dx, tribe.capitalY + dy, kind, 6);
          planned += 1;
        }
  ...
  private generateCraftingPlans(tribe: TribeState): void {
    if (tribe.age >= AgeType.Stone && tribe.resources[ResourceType.Wood] > 20 && tribe.resources[ResourceType.Stone] > 10) {
      this.enqueueCraftJob(tribe, ResourceType.StoneTools, 4, { [ResourceType.Wood]: 4, [ResourceType.Stone]: 4 }, BuildingType.Workshop);
    }
  ...
  private chooseTradeCargo(tribe: TribeState): ResourceType {
    if (tribe.resources[ResourceType.Planks] > 30) return ResourceType.Planks;
    if (tribe.resources[ResourceType.Wood] > 80) return ResourceType.Wood;
    if (tribe.resources[ResourceType.Stone] > 70) return ResourceType.Stone;
    if (tribe.resources[ResourceType.Ore] > 50) return ResourceType.Ore;
    if (tribe.resources[ResourceType.Fish] > 20) return ResourceType.Fish;
    if (tribe.resources[ResourceType.Grain] > 20) return ResourceType.Grain;
    ...
    for (let dy = -radius; dy <= radius && count < limit; dy += 1) {
      for (let dx = -radius; dx <= radius && count < limit; dx += 1) {
        ...
        this.jobs.push({
          ...
          x,
          y,
          priority: 4 + Math.max(0, radius - manhattan(tribe.capitalX, tribe.capitalY, x, y)) * 0.1,
          claimedBy: null,
        });
        count += 1;
      }
  ...
  private enqueueNearbyWaterJobs(tribe: TribeState, kind: JobKind, limit: number): void {
    let count = 0;
    for (let dy = -MAX_JOB_RADIUS; dy <= MAX_JOB_RADIUS && count < limit; dy += 1) {
      for (let dx = -MAX_JOB_RADIUS; dx <= MAX_JOB_RADIUS && count < limit; dx += 1) {
        const x = tribe.capitalX + dx;
        const y = tribe.capitalY + dy;
        if (!inBounds(x, y, this.world.width, this.world.height)) continue;
        const index = indexOf(x, y, this.world.width);
  ...
  private tribeTechs(tribe: TribeState): string[] {
    ...
    if (tribe.race.type === RaceType.Elves) techs.push("Grove Magic", "Moon Arrows", "Living Timber", "Spirit Wards", "Star Gardens", "Leaf Weaving", "Sunfire Circles", "Archmage Conclaves", "Skyglass Lenses", "Canopy Beacons");
    if (tribe.race.type === RaceType.Darkfolk) techs.push("Shadow Rites", "Ash Sorcery", "Blood Oaths", "Dread Wards", "Nightfire", "Hex Libraries", "Void Lenses", "Night Cataclysms", "Umbral Coils", "Black Lantern Flight");
    if (tribe.race.type === RaceType.Dwarves) techs.push("Deep Masonry", "Vault Locks", "Runed Plate", "Tunnel Warfare", "Mountain Clinics", "Bolt Throwers", "Hall Carving", "Thunder Forges", "Breach Guns", "Steam Hammers", "Tunnel Rails", "Rail Carbines", "Sky Dockyards");
    if (tribe.race.type === RaceType.Darkfolk) techs.push("Underpaths");
    if (tribe.race.type === RaceType.Orcs) techs.push("War Drums", "Brutal Charges", "Ash Steel", "Siege Rush", "War Breeding", "Blood Smithing");
    if (tribe.race.type === RaceType.Humans) techs.push("Charters", "Civic Markets", "Mercenary Bands", "Banner Diplomacy", "Stone Hospices", "Road Levies", "Arquebusiers", "Powder Trains", "Iron Rails", "Factory Charters", "Rifle Columns", "Electric Dispatch", "Air Marshals");
    if (tribe.race.type === RaceType.Halflings) techs.push("Herbal Tonic", "Seed Banks", "Hearth Feasts", "Pony Carts", "Cider Pressing", "Hedgerow Medicine");
    if (tribe.race.type === RaceType.Nomads) techs.push("Dune Paths", "Skirmish Riding", "Water Finding", "Leatherwork", "Field Triage", "Sky Watch");
    if (tribe.race.type === RaceType.Goblins) techs.push("Scrap Craft", "Trap Pits", "Night Raids", "Burrow Stores", "Sting Bows", "Rope Towers", "Spark Powder", "Crank Guns", "Smoke Mills");
    return techs;
  }
  ...
  private waterCapacityForTribe(tribeId: number, population: number): number {
    ...
    return Math.floor(24 + cisterns * 64 + canals * 6 + wellsOfTown * 18 + pumps * 28 + population * 1.5);
  }
  ...
  private waterIncomeForTribe(tribeId: number, weather: WeatherKind): number {
    ...
    const nearbyWater = hasAdjacentWater(this.world, tribe.capitalX, tribe.capitalY, 10) ? 5 : 0;
    ...
    const weatherGain =
      ...
      : 0;
    return Math.max(0, Math.floor(cisterns * 2.5 + canals * 0.6 + waterworks * 0.35 + pumps * 1.8 + surfaceRunoff * 0.4 + nearbyWater + springMelt + weatherGain));
  }
  ...
  private waterDemandForTribe(tribeId: number, population: number, weather: WeatherKind): number {
    ...
    const baseline = population * 0.45 + farms * 1.6 + orchards * 1.1 + stables * 0.7;
    const weatherPressure =
      weather === WeatherKind.Heatwave ? 8
  ...
  private serializeTribes(): TribeSummary[] {
    return this.tribes.map((tribe) => {
      ...
      return {
        ...
        enemyCount: tribe.relations.filter((score, index) => index !== tribe.id && diplomacyStateFromScore(score) >= DiplomacyState.Hostile).length,
